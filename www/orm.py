...
class Model(dict, metaclass = ModelMetaclass):
    ...
    async def save(self):
        ...
        if rows != 1:
            logging.warn('Failed to instert record: affected row: %s', rows)
    ...
    async def remove(self):
        ...
        if rows != 1:
            logging.warn('Failed to remove by primary key: affectd rows: %s', rows)